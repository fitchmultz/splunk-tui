...
Repository-local composite GitHub Actions for Splunk operations in CI/CD pipelines. These actions enable workflows that interact with Splunk Enterprise for validation, testing, monitoring, and data ingestion.
...
For production workflows, prefer pinning `image-tag` to a release tag (for example `v0.4.0`) instead of `latest` for deterministic behavior.
