...
Repository-local composite GitHub Actions for integrating Splunk operations into CI/CD pipelines. These actions enable teams to validate configurations, run searches, check health, and send events to Splunk directly from GitHub workflows.
...
> For deterministic pipelines, pin action `image-tag` inputs to a release tag (for example `v0.4.0`) instead of relying on `latest`.
