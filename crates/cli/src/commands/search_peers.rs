...
/// Run the search-peers command.
///
/// Lists distributed search peers from the Splunk search head.
///
/// # Arguments
///
...
pub async fn run(
    ...
    info!(
        "Listing search peers (count: {}, offset: {})",
        count, offset
    );
    ...
    let peers = cancellable!(client.list_search_peers(Some(count), offset_param), cancel)?;
    ...
    if format == OutputFormat::Table {
        ...
        let output = formatter.format_search_peers_paginated(&peers, detailed, pagination)?;
        output_result(&output, format, output_file.as_ref())?;
        return Ok(());
    }

    let formatter = get_formatter(format);
    let output = formatter.format_search_peers(&peers, detailed)?;
    output_result(&output, format, output_file.as_ref())?;
