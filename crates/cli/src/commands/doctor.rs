...
/// Write a single entry to a zip archive with proper error context.
///
/// # Arguments
/// * `zip` - Mutable reference to the ZipWriter
/// * `entry_name` - Name of the entry within the zip archive
/// * `options` - Zip file options (compression method, etc.)
/// * `contents` - Byte content to write to the entry
///
/// # Errors
/// Returns an error with context if starting the file or writing contents fails.
    ...
    for log_dir in log_dirs {
        if log_dir.exists() {
            ...
            let mut entries: Vec<_> = std::fs::read_dir(&log_dir)
                .with_context(|| {
                    format!("Failed to read TUI log directory at {}", log_dir.display())
                })?
                .flatten()
                .collect();
            ...
            if let Some(latest) = entries.last() {
                ...
                let content = std::fs::read_to_string(&path).with_context(|| {
                    format!("Failed to read TUI log file at {}", path.display())
                })?;
