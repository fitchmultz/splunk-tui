//! Authentication retry behavior tests.
//!
//! This module tests the client's retry logic for authentication-related errors
//! (401 Unauthorized, 403 Forbidden) when using session token authentication.
//!
//! # Invariants
//! - 401/403 responses trigger session re-authentication ONLY for SessionToken auth strategy
//! - API token authentication does NOT retry on 401 (token is either valid or not)
//! - After re-authentication, the original request is retried
//! - If 401/403 persists after re-login, the error is returned
//!
//! # What this does NOT handle
//! - Rate limiting retries (see retry_rate_limit_tests.rs)
//! - Server error retries (see retry_server_error_tests.rs)
//! - Initial login failure handling
