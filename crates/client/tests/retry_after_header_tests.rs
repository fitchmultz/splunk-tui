...
//! which can be specified as either delta-seconds or HTTP-date format.
//!
//! # Invariants
//! - Retry-After header in delta-seconds format is respected
//! - Retry-After header in HTTP-date format is respected
//! - When Retry-After specifies a time shorter than exponential backoff, the larger value is used
//! - When Retry-After header is missing, exponential backoff is used
//! - When Retry-After header is invalid, exponential backoff is used as fallback
//! - Past HTTP-dates fall back to exponential backoff
//!
//! # What this does NOT handle
//! - Basic 429 retry behavior (see retry_rate_limit_tests.rs)
