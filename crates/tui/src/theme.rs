//! Runtime theme types for Splunk TUI rendering.
//!
//! Responsibilities:
...
//!
//! Does NOT handle:
//! - Theme persistence (see `splunk-config` which persists `ColorTheme`).
//! - Widget composition or rendering decisions (see `ui` and feature modules).
//!
//! Invariants:
//! - `ColorTheme` lives in `splunk-config`; `Theme` is the runtime expansion used only by the TUI.
//! - `Theme` is intentionally NOT serializable.
